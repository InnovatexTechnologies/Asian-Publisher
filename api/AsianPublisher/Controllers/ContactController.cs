...
namespace AsianPublisher.Controllers
{
    ...
    public class ContactController : Controller
    {
        ...
        public ContactController(IConfiguration _configuration, ILogger<ContactController> logger, IWebHostEnvironment _webHost)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<Contact> records = Contact.Get(Utility.FillStyle.AllProperties, param).OrderBy(o => o.name).ToList();
                return View(records);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IActionResult Create(Contact model, string url)
        {
            if (model.Save())
            {
                return Redirect(url);
        ...
        {
            Contact model = Contact.GetById(id, Utility.FillStyle.AllProperties);
            ViewBag.url = url;
            if (model == null)
            {
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Edit(Contact model, string url)
        {
            if (model.Update())
            {
                return Redirect(url);
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Delete(Contact model, string url)
        {
        ...
        {
            Dictionary<string, string> param = new Dictionary<string, string>();
            List<Contact> model = Contact.Get(Utility.FillStyle.AllProperties, param);
            int v = 1;
    ...
    new DataColumn("Message"),
        ...
        public FileContentResult GeneratePdf(string id)
        {
            ...
            iTextSharp.text.Paragraph report = new iTextSharp.text.Paragraph("Contact", fontb);
            ...
            return File(bytea, "application/pdf", "Contact.pdf");
        }
    }
}
