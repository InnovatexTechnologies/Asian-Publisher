...
namespace AsianPublisher.Controllers
{
    ...
    public class CourseController : Controller
    {
        ...
        public CourseController(IConfiguration _configuration, ILogger<CourseController> logger, IWebHostEnvironment _webHost)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<Course> records = Course.Get(Utility.FillStyle.AllProperties, param).OrderBy(o => o.name).ToList();
                return View(records);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IActionResult Create(Course model, string url)
        {
            if (model.Save())
            {
                return Redirect(url);
        ...
        {
            Course model = Course.GetById(id, Utility.FillStyle.AllProperties);
            ViewBag.url = url;
            if (model == null)
            {
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Edit(Course model, string url)
        {
            if (model.Update())
            {
                return Redirect(url);
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Delete(Course model, string url)
        {
        ...
        {
            Dictionary<string, string> param = new Dictionary<string, string>();
            List<Course> model = Course.Get(Utility.FillStyle.AllProperties, param);
            int v = 1;
    ...
    new DataColumn("Name"),
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Course.xlsx");
                }
            }
        }
        public FileContentResult GeneratePdf(string id)
        {
            ...
            iTextSharp.text.Paragraph report = new iTextSharp.text.Paragraph("Courses", fontb);
            ...
            return File(bytea, "application/pdf", "Course.pdf");
        }
    }
}
