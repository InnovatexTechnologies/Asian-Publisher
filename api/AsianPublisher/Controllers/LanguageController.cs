...
namespace AsianPublisher.Controllers
{
    ...
    public class LanguageController : Controller
    {
        ...
        public LanguageController(IConfiguration _configuration, ILogger<LanguageController> logger, IWebHostEnvironment _webHost)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<Language> records = Language.Get(Utility.FillStyle.AllProperties, param).OrderBy(o => o.name).ToList();
                return View(records);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IActionResult Create(Language model, string url)
        {
            if (model.Save())
            {
                return Redirect(url);
        ...
        {
            Language model = Language.GetById(id, Utility.FillStyle.AllProperties);
            ViewBag.url = url;
            if (model == null)
            {
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Edit(Language model, string url)
        {
            if (model.Update())
            {
                return Redirect(url);
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Delete(Language model, string url)
        {
        ...
        {
            Dictionary<string, string> param = new Dictionary<string, string>();
            List<Language> model = Language.Get(Utility.FillStyle.AllProperties, param);
            int v = 1;
    ...
    new DataColumn("Name"),
            ...
            foreach (Language obj1 in model)
            {
                dt.Rows.Add(
                    v
            , obj1.name
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Language.xlsx");
                }
            }
        }
        public FileContentResult GeneratePdf(string id)
        {
            ...
            iTextSharp.text.Paragraph report = new iTextSharp.text.Paragraph("Languages", fontb);
            ...
            return File(bytea, "application/pdf", "Language.pdf");
        }
    }
}
