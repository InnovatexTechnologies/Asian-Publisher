...
{
    [AllowAnonymous]
    public class SemesterCategoryController : Controller
    {
        private IConfiguration configuration;
        private readonly IWebHostEnvironment webHost;
        private readonly ILogger<SemesterCategoryController> _logger;

        public SemesterCategoryController(IConfiguration _configuration, ILogger<SemesterCategoryController> logger, IWebHostEnvironment _webHost)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<SemesterCategory> records = SemesterCategory.Get(Utility.FillStyle.AllProperties, param).OrderBy(o => o.name).ToList();
                return View(records);
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Create(string url)
        {
            try
            {
                ...
                ViewBag.url = url;
                return View(new SemesterCategory());
            }
            catch (Exception ex)
            {
                // Log the exception or handle it as needed
        ...
        [HttpPost]
        public IActionResult Create(SemesterCategory model, string url)
        {
            if (model.Save())
            {
                return Redirect(url);
        ...
        {
            SemesterCategory model = SemesterCategory.GetById(id, Utility.FillStyle.AllProperties);
            ViewBag.url = url;
            if (model == null)
            {
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Edit(SemesterCategory model, string url)
        {
            if (model.Update())
            {
                return Redirect(url);
        ...
        // POST: Create a new record
        [HttpPost]
        public IActionResult Delete(SemesterCategory model, string url)
        {
        ...
        {
            Dictionary<string, string> param = new Dictionary<string, string>();
            List<SemesterCategory> model = SemesterCategory.Get(Utility.FillStyle.AllProperties, param);
            int v = 1;
            DataTable dt = new DataTable("SemesterCategories");
            dt.Columns.AddRange(new DataColumn[2]
            {
    new DataColumn("S.No.",typeof(int)),
    new DataColumn("Name"),
            });
            foreach (SemesterCategory obj1 in model)
            {
                dt.Rows.Add(
                    v
            , obj1.name
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "SemesterCategory.xlsx");
                }
            }
        }
        public FileContentResult GeneratePdf(string id)
        {
            ...
            iTextSharp.text.Paragraph report = new iTextSharp.text.Paragraph("SemesterCategories", fontb);
            ...
            return File(bytea, "application/pdf", "SemesterCategory.pdf");
        }
    }
}
